...
class AdnoViewer extends Component {
    ...
    componentDidMount() {
        projectDB.exists(this.props.match.params.id)
            .then(project => {
                if (!project) {
                ...
                if (project.annotations && project.annotations.length > 0) {
                    const newAnnos = project.annotations.map(annotation => {
                        const newAnno = annotation;
                        newAnno.body = annotation.body.filter(annoBody => annoBody.type !== "AdnoRichText");
                        return newAnno
                    })

                    this.setState({ annotations: project.annotations, selectedProject: project })
